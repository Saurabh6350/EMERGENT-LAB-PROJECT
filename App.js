...
// ─── AI Chart Insight Panel (Groq-powered) ─────────────────────────────────────
function ChartInsight({chartType,xCol,yCol,labelCol,valueCol,col,colorCol,rows,colTypes,theme,dataName}){
  const T=themes[theme];
  const[insight,setInsight]=useState(null);
  const[loading,setLoading]=useState(false);
  const[expanded,setExpanded]=useState(false);

  const computeCorr=(aName,bName)=>{
    if(!aName||!bName||!rows)return null;
    const av=rows.map(r=>Number(r[aName])).filter(v=>!isNaN(v));
    const bv=rows.map(r=>Number(r[bName])).filter(v=>!isNaN(v));
    ...
    const dx=Math.sqrt(av.slice(0,n).reduce((s,v)=>s+(v-am)**2,0));
    const dy=Math.sqrt(bv.slice(0,n).reduce((s,v)=>s+(v-bm)**2,0));
    return dx&&dy?parseFloat((num/(dx*dy)).toFixed(3)):null;
  };

  const buildContext=()=>{
    const numCols=(colTypes||[]).filter(c=>c.type==="number").slice(0,8);
    const catCols=(colTypes||[]).filter(c=>c.type==="string").slice(0,4);
    const numStats=numCols.map(c=>{
      const vals=(rows||[]).map(r=>Number(r[c.name])).filter(v=>!isNaN(v));
      if(!vals.length)return"";
      const s=[...vals].sort((a,b)=>a-b);
      const mean=(vals.reduce((a,v)=>a+v,0)/vals.length).toFixed(2);
      const std=Math.sqrt(vals.reduce((a,v)=>a+(v-mean)**2,0)/vals.length).toFixed(2);
      return`${c.name}: mean=${mean}, min=${s[0].toFixed(2)}, max=${s[s.length-1].toFixed(2)}, std=${std}, n=${vals.length}`;
    }).filter(Boolean).join("\n");
    const catStats=catCols.map(c=>{
      const cnt={};(rows||[]).forEach(r=>{const v=r[c.name];if(v)cnt[v]=(cnt[v]||0)+1;});
      return`${c.name}: `+Object.entries(cnt).sort((a,b)=>b[1]-a[1]).slice(0,5).map(([k,v])=>`${k}(${v})`).join(", ");
    }).join("\n");

    let chartDesc="";
    if(chartType==="scatter"&&xCol&&yCol){
      const r=computeCorr(xCol,yCol);
      const strength=r==null?"unknown":Math.abs(r)>0.7?"strong":Math.abs(r)>0.4?"moderate":"weak";
      const dir=r==null?"":r>0?" positive":" negative";
      chartDesc=`Scatter plot of ${yCol} (y-axis) vs ${xCol} (x-axis).${r!=null?` Pearson r=${r} — ${strength}${dir} correlation.`:""}${colorCol?` Points colored by ${colorCol}.`:""}`;
    } else if(chartType==="line"&&xCol&&yCol){
      chartDesc=`Line chart showing ${yCol} over ${xCol} (${(rows||[]).length} data points).`;
    } else if(chartType==="bar"&&col){
      const cnt={};(rows||[]).forEach(r=>{const v=r[col]||"(empty)";cnt[v]=(cnt[v]||0)+1;});
      const top=Object.entries(cnt).sort((a,b)=>b[1]-a[1]).slice(0,3).map(([k,v])=>`"${k}" (${v})`).join(", ");
      chartDesc=`Bar chart of ${col} distribution. Top categories: ${top}.`;
    } else if(chartType==="pie"&&labelCol&&valueCol){
      const cnt={};(rows||[]).forEach(r=>{const k=r[labelCol];const v=Number(r[valueCol])||0;cnt[k]=(cnt[k]||0)+v;});
      const top=Object.entries(cnt).sort((a,b)=>b[1]-a[1]).slice(0,3).map(([k,v])=>`"${k}" (${v.toFixed(1)})`).join(", ");
      chartDesc=`Pie chart of ${valueCol} by ${labelCol}. Top segments: ${top}.`;
    } else if(chartType==="histogram"&&col){
      const vals=(rows||[]).map(r=>Number(r[col])).filter(v=>!isNaN(v));
      const mean=vals.length?(vals.reduce((a,v)=>a+v,0)/vals.length).toFixed(2):"n/a";
      const s=[...vals].sort((a,b)=>a-b);
      chartDesc=`Histogram showing distribution of ${col}. Mean=${mean}, min=${s[0]?.toFixed(2)}, max=${s[s.length-1]?.toFixed(2)}, n=${vals.length}.`;
    } else if(chartType==="heatmap"){
      chartDesc=`Correlation heatmap across all numeric columns in the dataset.`;
    }

    return`Dataset: "${dataName||"data"}" — ${(rows||[]).length} rows, ${(colTypes||[]).length} columns.\n\nChart: ${chartDesc}\n\nNumeric column stats:\n${numStats}\n\nCategorical columns:\n${catStats}`;
  };
  ...
  const handleGenerate=async()=>{
    ...
    try{
      ...
      const text=await callAI([{role:"user",content:`You are analyzing a data visualization. Given the chart data below, write a detailed AI insight that explains:\n1. What this chart shows and how to read it\n2. Key patterns, trends, or anomalies visible in the data\n3. The relationship between the variables (if applicable) — direction, strength, meaning\n4. What the numbers actually mean — reference specific values\n5. A concrete, actionable recommendation based on this chart\n\nBe specific, reference actual numbers, and write in clear business English. Aim for 4-6 sentences total.\n\n${ctx}`}],"You are a senior data analyst. Write clear, data-specific insights with actual numbers and actionable recommendations.");
      setInsight(text);setExpanded(true);
    }catch(e){setInsight("⚠️ Could not generate insight: "+e.message);setExpanded(true);}
    setLoading(false);
  };

  return(
    <div style={{marginTop:16,borderTop:`1px solid ${T.border}`,paddingTop:14}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:insight?10:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:18,height:18,borderRadius:4,background:"linear-gradient(135deg,#6ee7b7,#818cf8)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,flexShrink:0}}>✦</div>
          <span style={{fontSize:11,fontWeight:700,color:T.muted,letterSpacing:"0.08em"}}>AI INSIGHT</span>
          {insight&&<span onClick={()=>setExpanded(e=>!e)} style={{fontSize:10,color:T.faint,cursor:"pointer",marginLeft:4}}>{expanded?"▲ collapse":"▼ expand"}</span>}
        </div>
        {!insight&&(
          <button onClick={handleGenerate} disabled={loading} style={{fontSize:10,padding:"5px 14px",background:loading?"transparent":"linear-gradient(135deg,rgba(110,231,183,0.15),rgba(129,140,248,0.15))",border:`1px solid ${loading?T.border:"rgba(110,231,183,0.4)"}`,borderRadius:6,color:loading?T.faint:"#6ee7b7",cursor:loading?"wait":"pointer",fontFamily:"inherit",fontWeight:700,letterSpacing:"0.06em",transition:"all 0.2s"}}>
            {loading?"ANALYZING...":"EXPLAIN THIS CHART →"}
          </button>
        )}
        {insight&&!loading&&(
          <button onClick={handleGenerate} style={{fontSize:10,padding:"4px 10px",background:"transparent",border:`1px solid ${T.border}`,borderRadius:6,color:T.faint,cursor:"pointer",fontFamily:"inherit"}}>↺ Regenerate</button>
        )}
      </div>
      {loading&&(
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",background:`linear-gradient(135deg,rgba(110,231,183,0.05),rgba(129,140,248,0.05))`,borderRadius:8,border:`1px solid ${T.border}`,marginTop:8}}>
          <div style={{display:"flex",gap:4}}>{[0,1,2].map(i=><div key={i} style={{width:6,height:6,borderRadius:"50%",background:"#6ee7b7",animation:`bounce 1s ease-in-out ${i*0.2}s infinite`}}/>)}</div>
          <span style={{fontSize:12,color:T.faint}}>Groq AI is analyzing this chart…</span>
        </div>
      )}
      {insight&&expanded&&(
        <div style={{background:`linear-gradient(135deg,rgba(110,231,183,0.04),rgba(129,140,248,0.04))`,border:`1px solid rgba(110,231,183,0.2)`,borderRadius:10,padding:"14px 16px",marginTop:4}}>
          <div style={{fontSize:13,color:T.muted,lineHeight:1.8,whiteSpace:"pre-wrap"}}>{insight}</div>
        </div>
      )}
    </div>
  );
            ...
            {activeTab==="charts"&&data&&(
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div style={{...mkCard(T),padding:22}}>
                  ...
                  {scatterX&&scatterY&&<><ScatterPlot rows={data.rows} xCol={scatterX} yCol={scatterY} colorCol={scatterC} theme={theme} title={`${scatterY} vs ${scatterX}`}/>
                  <ChartInsight key={`scatter-${scatterX}-${scatterY}`} chartType="scatter" xCol={scatterX} yCol={scatterY} colorCol={scatterC} rows={data.rows} colTypes={data.colTypes} theme={theme} dataName={data.name}/></>}</>)}
                  ...
                  {lineX&&lineY&&<><LineChart rows={data.rows} xCol={lineX} yCol={lineY} theme={theme} title={`${lineY} over ${lineX}`}/>
                  <ChartInsight key={`line-${lineX}-${lineY}`} chartType="line" xCol={lineX} yCol={lineY} rows={data.rows} colTypes={data.colTypes} theme={theme} dataName={data.name}/></>}</>)}
                  {chartMode==="bar"&&(<><div style={{...mkLbl(T),marginBottom:14}}>CATEGORICAL DISTRIBUTIONS</div>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:18}}>
                      {catCols.slice(0,4).map((col,ci)=>{const cnt={};data.rows.forEach(r=>{const v=r[col.name]||"(empty)";cnt[v]=(cnt[v]||0)+1;});const sorted=Object.entries(cnt).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([label,count])=>({label,count}));
                        return<div key={col.name} style={{...mkCard(T),padding:16}}>
                          <div style={{display:"flex",justifyContent:"flex-end",marginBottom:4}}><button onClick={()=>addToDashboard({type:"bar",col:col.name,title:`${col.name} distribution`,color:COLORS[ci%COLORS.length]})} style={{fontSize:10,background:"transparent",border:"none",color:T.faint,cursor:"pointer"}}>📌</button></div>
                          <BarChart data={sorted} labelKey="label" valueKey="count" color={COLORS[ci%COLORS.length]} theme={theme} title={col.name+" distribution"}/>
                          <ChartInsight key={`bar-${col.name}`} chartType="bar" col={col.name} rows={data.rows} colTypes={data.colTypes} theme={theme} dataName={data.name}/>
                        </div>;})}
                    </div></>)}
                  ...
                  {pieLabel&&pieValue&&(()=>{const cnt={};data.rows.forEach(r=>{const k=r[pieLabel];const v=Number(r[pieValue])||0;cnt[k]=(cnt[k]||0)+v;});const d2=Object.entries(cnt).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([label,value])=>({label,value}));return<><PieChart data={d2} labelKey="label" valueKey="value" theme={theme} title={`${pieValue} by ${pieLabel}`}/><ChartInsight key={`pie-${pieLabel}-${pieValue}`} chartType="pie" labelCol={pieLabel} valueCol={pieValue} rows={data.rows} colTypes={data.colTypes} theme={theme} dataName={data.name}/></>;})()}</>)}
                  {chartMode==="heatmap"&&<><Heatmap rows={data.rows} colTypes={data.colTypes} theme={theme}/><ChartInsight key="heatmap" chartType="heatmap" rows={data.rows} colTypes={data.colTypes} theme={theme} dataName={data.name}/></>}
                  {chartMode==="histogram"&&(<><div style={{...mkLbl(T),marginBottom:14}}>NUMERIC DISTRIBUTIONS</div>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:16}}>
                      {numCols.slice(0,6).map(col=><div key={col.name} style={{...mkCard(T),padding:16}}>
                        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:4}}><button onClick={()=>addToDashboard({type:"histogram",col:col.name,title:`${col.name} distribution`})} style={{fontSize:10,background:"transparent",border:"none",color:T.faint,cursor:"pointer"}}>📌</button></div>
                        <Histogram rows={data.rows} col={col.name} theme={theme}/>
                        <ChartInsight key={`hist-${col.name}`} chartType="histogram" col={col.name} rows={data.rows} colTypes={data.colTypes} theme={theme} dataName={data.name}/>
                      </div>)}
                    </div></>)}
                </div>
              </div>
                ...
                <div style={{...mkCard(T),padding:22}}>
                  ...
                  {aiChartConfig&&(
                    <div>
                      ...
                      <ChartInsight
                        key={`ai-${aiChartConfig.type}-${aiChartConfig.xCol||aiChartConfig.col||aiChartConfig.labelCol}`}
                        chartType={aiChartConfig.type}
                        xCol={aiChartConfig.xCol} yCol={aiChartConfig.yCol}
                        col={aiChartConfig.col} labelCol={aiChartConfig.labelCol}
                        valueCol={aiChartConfig.valueCol} colorCol={aiChartConfig.colorCol}
                        rows={data.rows} colTypes={data.colTypes} theme={theme} dataName={data.name}/>
                    </div>
                  )}
                </div>
                <div style={{...mkCard(T),padding:22}}>
